...
<section id="lapangan" class="container content">
    ...
    <div class="grid-container">
        <?php
        $query = mysqli_query($conn, "SELECT * FROM lapangan LIMIT 4");
        while($row = mysqli_fetch_assoc($query)) {
        ?>
            <div class="card popular-card">
                <div class="card-detail">
                    <span class="tag"><?php echo $row['Jenis_olahraga']; ?></span>
                    <h4><?php echo $row['Nama_lapangan']; ?></h4>
                    <p>Jl. <?php echo $row['Lokasi']; ?></p>
                    <div class="card-meta">
                        <span>⭐ 4.8</span>
                        <span><?php echo isset($row['Harga']) ? 'Rp'.number_format($row['Harga'],0,',','.').'/jam' : 'Mulai dari Rp50.000/jam'; ?></span>
                    </div>
